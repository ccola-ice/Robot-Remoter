...
主菜单第 10 项 **Hardware Tests** 和第 11 项 **EEPROM** 分别进入硬件测试与 EEPROM 页面。两个页面以中文显示，协议名、器件名、按键名和单位等必要内容保留英文；诊断串口日志仍使用 ASCII，避免终端编码影响排错。
...
需要人工操作、仪表或对端配合的项目放在 Hardware Tests 内按需执行。Flash、EEPROM 和 SD 卡读写检测改为每次开机自动执行，不再出现在手动测试列表。前台测试期间常规菜单任务暂停；不要在控制机器人运行时进行测试。退出后恢复正常菜单。测试列表保存本次上电各项结果，取消不覆盖上次结果。
...
| NRF diagnostic RX | 在本机启动后，20 秒内让测试对端发送。收到并比较带连续序号的 8 个包才通过。BACK 取消。两端使用通道 40、1 Mbps、固定 32 字节、2 字节 CRC、自动 ACK；地址寄存器按顺序写入 `D7 43 44 47 31`。正常通信地址不会收到这些测试包。 |
| Flash 专用扇区读写（开机自动） | 先检测 Flash，再检查专用扇区全 FF；分 256 字节页写入不同图案、读回比较、擦除并检查恢复 FF。任一步失败即 FAIL。发现非 FF 时失败且不会擦除。 |
| EEPROM 保留字节读写（开机自动） | 仅测试 AT24C08 首块的 `0xFF`，要求原值为 FF；写 55/AA 分别读回，再恢复 FF 并验证。任一步失败即 FAIL；原值非 FF 时不写入。 |
| SD 临时文件读写（开机自动） | 使用 `FA_CREATE_NEW` 选择未占用的 `0:/D000.TMP` 至 `D999.TMP`，写入 4 KiB 图案、同步、关闭重开、完整比较、关闭并删除本次新建文件。短读写、关闭/清理失败均报 FAIL；没有空闲文件名时失败。不会格式化、截断旧文件或测试原始扇区。 |
...
无线测试开始前要求 FIFO 为空，避免丢弃正常通信数据；空 FIFO 下的历史中断标志会自动清除。发送端使用较宽松的自动重发间隔并对失败包有限次重试，接收端同时检查中断标志和 RX FIFO，避免漏掉已入队数据。退出时恢复频道、速率、地址、自动 ACK、动态载荷、工作模式和 CE 状态并核对寄存器，无法恢复同样报 FAIL。

开机存储写入测试期间请保持供电。断电可能留下专用区的测试数据或新建的 TMP 文件；下次启动遇到非空专用区会停止，不自动擦除来历不明的数据。代码没有整片擦除或格式化入口。
...
4. `0xFF` 保留给开机自动写入测试，浏览可读，编辑禁止。没有批量清空入口。
