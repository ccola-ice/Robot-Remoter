...
#define      ILI9806G_RST_PIN               GPIO_Pin_7
...
void                     	ILI9806G_SetPointPixel           ( uint16_t usX, uint16_t usY );		//底层画点函数
uint16_t                 	ILI9806G_GetPointPixel           ( uint16_t usX , uint16_t usY );
void                     	ILI9806G_DrawLine                ( uint16_t usX1, uint16_t usY1, uint16_t usX2, uint16_t usY2 );	//底层画线函数
...
void						LCD_SetTextColor		(uint16_t Color);  //设置前景(文字)颜色
void						LCD_SetColors			(uint16_t TextColor, uint16_t BackColor); 	//设置前景和背景颜色，本质就是修改那个全局变量
void						LCD_GetColors			(uint16_t *TextColor, uint16_t *BackColor); //获取当前的前景颜色和背景颜色。
