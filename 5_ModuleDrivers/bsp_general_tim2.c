...
static void GENERAL_TIM2_NVIC_Config(void)
{
    ...
    NVIC_InitStructure.NVIC_IRQChannel = GENERAL_TIM2_IRQn;// 设置中断来源 		
 ...
 *-----------------------------------------------------------------------------
 */
static void GENERAL_TIM2_Mode_Config(uint16_t arr,uint16_t psc)
{
...
{
    GENERAL_TIM2_Mode_Config(arr,psc);
